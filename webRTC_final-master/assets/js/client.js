...
const MAXIMUM_MESSAGE_SIZE = 1000;
    ...
    if (success === false) {
        ...
        navigator.getUserMedia({
            ...
            myConn.ondatachannel = function(event) {
                ...
                if(receiveChannel.label == 'channel1') {
                    receiveChannel.onmessage = function(event) {
                    console.log("ondatachannel message:", event.data);
                    chatArea.innerHTML += "<div class='left-align' style='display:flex;align-items:center;'><div style='font-weight:600;margin:0 5px;'>" + connected_user + "</div>: <div>" + event.data + "</div></div><br/>";
                    }
                }
                else {
                        receiveChannel.onmessage = function(event){
                        ...
                        try {
                            ...
                            receivedBuffers.push(data.message);
                            console.log(data.message);
                            if(data.last) {
                                console.log(receivedBuffers.join(''));
                                receivedFile = document.getElementById("receivedFile");
                                receivedFile.innerHTML = "<button class='webrtc-login btn-large waves-effect waves-dark'>Download</button>";
                                receivedFile.onclick = function(){
                                downloadFile(receivedBuffers.join(''), receiveChannel.label);
                                receivedBuffers = [];
                                }
                            }
...
msgSendBtn.addEventListener("click", function (event) {
    ...
    chatArea.innerHTML += "<div class='right-align' style='display:flex; align-items:center;align-self:flex-end'><div>" + msgVal + "</div>:<div style='font-weight:600;margin:0 5px;'> " + name + "</div></div><br/>";
    dataChannel.send(msgVal);
    msgInput.value = "";
})

function onSendData(channel, text) {
    var data = {};
    if(text.length > MAXIMUM_MESSAGE_SIZE){
        data.message = text.slice(0, MAXIMUM_MESSAGE_SIZE);
    }
    else {
        data.message = text;
        data.last = true;
    ...
    if(remainingData.length) {
        setTimeout(function(){
            onSendData(channel, remainingData)
        }, 0)
    }
}
...
fileInput.onchange = function(e) {
    const channel = myConn.createDataChannel(this.files[0].name);

    channel.onopen = () => {
        var fr=new window.FileReader();
        fr.readAsDataURL(this.files[0]);
        fr.onload = function(event, text){
            onSendData(channel, event.target.result);
        }
    };
